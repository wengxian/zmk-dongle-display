...
static void set_modifiers(lv_obj_t *widget, struct modifiers_state state) {
    ...
    if (index == 1) {
        memcpy(current_modifier_symbols, mac_modifier_symbols, sizeof(mac_modifier_symbols));
    } else {
        memcpy(current_modifier_symbols, win_modifier_symbols, sizeof(win_modifier_symbols));
    }
